...
public class ScheduleRepository {
    ...
    public Optional<ScheduleTime> get(Integer id) {
        final String sql = "SELECT id, start_time " +
                "FROM bank_transactions.schedule " +
                "WHERE id = ?";
    ...
    public List<ScheduleTime> all() {
        final String sql = "SELECT id, start_time FROM bank_transactions.schedule";
        return jdbc.query(sql, (rs, rowNum) -> getScheduleTime(rs));
    }

    public void delete(Integer id) {
        final String sql = "DELETE FROM bank_transactions.schedule WHERE id = ?";
        jdbc.update(sql, id);
    }

    public void save(ScheduleTime scheduleTime) {
        final String sql = "INSERT INTO bank_transactions.schedule(start_time) VALUES (?)";
